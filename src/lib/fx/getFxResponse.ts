...
let lastGood: LastGood | null = loadLastGood();

function loadLastGood(): LastGood | null {
  try {
    const raw = readFileSync(LAST_GOOD_PATH, 'utf8');
    ...
    // No last-good yet — never invent a baked fallback rate.
  }
  return null;
}

function persistLastGood(value: LastGood) {
  lastGood = value;
  try {
    mkdirSync(dirname(LAST_GOOD_PATH), { recursive: true });
    writeFileSync(LAST_GOOD_PATH, JSON.stringify(value));
  } catch {
    // /tmp may be unavailable in some runtimes; in-memory last-good still applies.
  }
}

export function formatSaoPauloTimestamp(date = new Date()): string {
  ...
  const offset = saoPauloOffset(date);
...
async function fetchCbnNgnPerBrl(_brlUsd: number): Promise<number | null> {
  // CBN publishes HTML/XML tables, not a stable public JSON mid-market feed.
  // Do not invent a CBN parser or synthetic CBN rate.
  return null;
}
...
export async function buildFxSnapshot(): Promise<Omit<FxPayload, 'suggestedCurrency' | 'country'>> {
  ...
  if (!primary) {
    if (lastGood) {
      const delayed: Omit<FxPayload, 'suggestedCurrency' | 'country'> = {
        base: 'BRL',
  ...
  const cbnNgn = await fetchCbnNgnPerBrl(primary.rates.USD);
  const ngnCheck = cbnNgn || secondary?.rates.NGN || null;
  ...
  if (!delayed) {
    persistLastGood({ rates: primary.rates, fetchedAt, source: primary.source });
  }
...
export function readCountryFromHeaders(headers: Headers | Record<string, string | string[] | undefined>): string | null {
...
export async function getFxResponse(headers: Headers | Record<string, string | string[] | undefined>): Promise<FxPayload> {
  const snapshot = await buildFxSnapshot();
  const country = readCountryFromHeaders(headers);
  const suggestedCurrency: DisplayCurrency = suggestCurrencyForCountry(country);
  return {
    ...snapshot,
    country,
    suggestedCurrency,
